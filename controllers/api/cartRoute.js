    ...
    if (existingCartItem) {
      if (Number(req.body.quantity) === 0) {
        ...
        await product.update({
          stock: product.stock + existingCartItem.quantity,
        });
        ...
        if (product.stock < quantityChange) {
          res
            .status(418)
            .json("Not enough stock to add that many to your cart");
          return;
        }
...
router.get("/:id", withAuth, async (req, res) => {
  try {
    ...
    if (!cartItemData) {
      ...
      res
        .status(200)
        .json({ message: "No product found with this id in your cart." });
      return;
    }
