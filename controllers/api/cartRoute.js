...
//     const userEmail = user.email;
    ...
    const vendorEmails = cartItems.map(
      (item) => item.product.vendor.user.email
    );
    let baseURL;
    ...
    await Promise.all(
     ...
     //      text: `Go stop by the Farmers' Market to pick up your latest purchase: ${salesLink}`,
     //    };
        console.log(mailOptions);

        return; //transporter.sendMail(userEmailOptions); //transporter.sendMail(mailOptions); // FIXME: Uncomment when we have it deployed or want to test the emails
      })
    );
