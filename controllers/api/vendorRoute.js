...
router.post("/addProduct", withAuth, async (req, res) => {
  try {
    ...
    if (existingVendorProduct) {
      res.status(404).json({
        message:
          "This product already exists in your inventory. Please use the update product feature.",
      });
      return;
    }
...
router.put("/products/:id", withAuth, async (req, res) => {
  ...
  try {
    ...
    if (
      existingVendorProductByName &&
      existingVendorProductByName.id !== existingVendorProduct.id
    ) {
      res.status(404).json({
        message:
          "This product already exists in your inventory. Please use the update product feature.",
    ...
    const updatedData = {
      name:
        req.body.name !== oldData.name ? req.body.name : "No changes to name",
      description:
        req.body.description !== oldData.description
          ? req.body.description
          : "No changes to description",
      price:
        req.body.price !== oldData.price
          ? req.body.price
          : "No changes to price",
      stock:
